...
function ttVideoPopup() {
  blocks.modalVideoProduct.on('show.bs.modal', function(e) {
    ...
    // Validación del tipo de video
    if (!attrType || ['youtube', 'vimeo', 'video'].indexOf(attrType) === -1) {
      console.error("Tipo de video no válido");
      return;
    }

    if (attrType === "youtube") {
      // Asegúrate de usar el formato de URL embebida para YouTube
      attr = attr.replace("https://www.youtube.com/watch?v=", "https://www.youtube.com/embed/");
    ...
    if (attrType === "video") {
      if (!attrPoster) {
        console.warn("No se proporcionó un póster para el video");
      }
      $('<div class="tt-video-block"><a href="#" class="link-video"></a><video class="movie" src="'+attr+'" poster="'+(attrPoster || 'default-poster.jpg')+'" allowfullscreen></video></div>').appendTo($(this).find('.modal-video-content'));
    }
        ...
        currentAjaxRequest = $.getJSON(searchURL + '&view=json', function(data) {
          ...
          if(data.results_count == 0) {
            ...
            $.each(data.results, function(index, item) {
              ...
              if(item.compare == "$ 0.00"){
                link.append('<div class="tt-description"><div class="tt-title">' + 
                          item.title + 
                ...
                link.append('<div class="tt-description"><div class="tt-title">' + 
                          ...
                          '<span class="money"> '+ item.price + '</span>' +
                          '<span class="moneyCompare"> '+ item.compare + '</span></div>' +
                          '</div></div>');
              }
              link.wrap('<li></li>');
